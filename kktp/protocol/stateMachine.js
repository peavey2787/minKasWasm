...
export class KKTPStateMachine {
    ...
    async connect(discovery, response) {
        try {
            const { session, mailboxId } = await establishSession(this.portal, discovery, response);
            ...
            this.kktp.myPubSig = this.isInitiator ? discovery.pub_sig : response.pub_sig_resp;
            this.kktp.peerPubSig = this.isInitiator ? response.pub_sig_resp : discovery.pub_sig;
    ...
    sendMessage(plaintext) {
        if (this.state !== KKTP_STATES.ACTIVE) throw new Error(`Cannot send in state: ${this.state}`);
    ...
    receiveMessage(msg) {
        ...
        while (this.kktp.buffer.length > 0 && this.kktp.buffer[0].seq === this.kktp.inboundSeq) {
