...
import { discoveryValidator, responseValidator } from './canonical/validator.js';
 ...
 */
export async function establishSession(portal, discovery, response, keyIndex = 0) {
    // 1. Schema & Signature Validation
    discoveryValidator.validate(discovery);
    responseValidator.validate(response);

    const discBody = canonicalize(prepareForSigning(discovery, { omitKeys: ['sig'], excludeMeta: true }));
    const respBody = canonicalize(prepareForSigning(response, { omitKeys: ['sig_resp'] }));

    const [isDValid, isRValid] = await Promise.all([
        portal.crypto.verifyMessage(discovery.pub_sig, discBody, discovery.sig),
        portal.crypto.verifyMessage(response.pub_sig_resp, respBody, response.sig_resp)
    ]);

    if (!isDValid || !isRValid) throw new Error("Handshake Failed: Invalid Signatures");

    // 2. VRF Binding Verification
    // Initiator Binding: H(pub_sig || pub_dh || sid)
    const initiatorVrfInput = discovery.pub_sig + discovery.pub_dh + discovery.sid;
    ...
    const responderVrfInput = response.initiator_pub_sig + response.initiator_pub_dh + 
    ...
    const peerDH = (discovery.pub_dh === session.myPublicKeyHex) 
    ...
    const mailboxInput = new Uint8Array(pubSigA.length + pubSigB.length + sidBytes.length);
