...
export function constructAAD(mailboxId, direction, seq) {
    ...
    if (direction !== "AtoB" && direction !== "BtoA") {
        throw new Error(`Invalid KKTP direction: ${direction}. Must be "AtoB" or "BtoA".`);
    }

    // 2. Normalize Mailbox ID to Raw Bytes
    const mailboxBytes = typeof mailboxId === 'string' 
    ...
    if (seqBig < 0n || seqBig > 18446744073709551615n) {
        throw new Error("Sequence number out of range for unsigned 64-bit integer.");
    }
    ...
    const aad = new Uint8Array(mailboxBytes.length + dirBytes.length + seqBytes.length);
