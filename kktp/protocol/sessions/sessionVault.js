...
import { bytesToHex } from "../../../wrapper/utilities/utilities.js";
import {
  extractResumeState,
  applyResumeState,
...
import { KKTPProtocol } from "../kktpProtocol.js";
...
export class SessionVault {
  ...
  async _restoreInitiatorSession(s, ctx, resumeState) {
    ...
    if (s.response) {
      ...
      if (!mailboxId) {
        console.warn("KKTP: restore initiator failed - no mailboxId");
        return;
      }
  ...
  async _restoreResponderSession(s, ctx, resumeState) {
    ...
    if (!mailboxId) {
      console.warn("KKTP: restore responder failed - no mailboxId");
      return;
    }
