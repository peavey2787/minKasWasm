...
import { KaspaClient } from '../transport/kaspa_client.js';

const client = new KaspaClient({ /* ...rpc config... */ });
...
```js
await intelligence.syncFrom(
  startHash,      // Block hash to start from
  logFn,          // Optional logging function
  { maxSeconds, minTimestamp } // Options
);
// Indexer will be fed blocks from the DAG walk.
```

### Find Payload in Forward DAG Walk

```js
const results = await intelligence.findPayload(
  startHash,      // Block hash to start from
  searchText,     // Text to search for in payloads
  'contains',     // Match mode: 'contains', 'equals', etc.
  { maxSeconds, minTimestamp, logFn }
);
// Returns array of matching blocks/transactions.
```

### Find Historical Block/Tx with Custom Matcher

```js
const results = await intelligence.findHistorical(
  startHash,      // Block hash to start from
  matchFn,        // (block) => boolean, custom matcher
  { maxSeconds, maxDepth, logFn }
);
// Returns array of matching blocks/transactions.
```
...
console.log('Payload search results:', results);
