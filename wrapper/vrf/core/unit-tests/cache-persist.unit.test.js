...
(async function runCachePersistUnitTests() {
  ...
  assert.ok(
    btcCache.blocks[0].hash === "abc",
    "BTC cache returns correct block",
  );
  ...
  assert.ok(
    qrngCache.provider === "anu",
    "QRNG cache returns correct provider",
  );
  assert.ok(
    Array.isArray(qrngCache.result.data),
    "QRNG cache returns data array",
  );
