...
export const NistVerifier = {
  async getPublicKey(pulse) {
    ...
    if (certId) {
      try {
        console.warn("NistVerifier: attempting certificate fetch", certId);
        ...
        console.error("NistVerifier: certificate import failed:", certErr);
      }
    }
    ...
    try {
      ...
      console.log("NistVerifier: n bytes =", rawN.length);
      console.log("NistVerifier: n trimmed bytes =", trimmedN.length);
      console.log("NistVerifier: e bytes =", rawE.length);
      try {
        const key = await crypto.subtle.importKey(
          "jwk",
          buildJwk(trimmedN),
  ...
  async verifyPulse(pulse) {
    try {
      ...
      if (!pulse.uri || !pulse.version || !pulse.certificateId) {
        console.error("NistVerifier: pulse missing required fields", {
          uri: pulse.uri,
