...
/**
 * Folding Module
 * Implements recursive folding rules for position updates
 * Only SHA-256 folding rule is supported
 */
...
export async function sha256FoldingRule(previousOutput, numPositions) {
    ...
    if (typeof previousOutput !== 'string' || !/^[01]+$/.test(previousOutput) || previousOutput.length < 8) {
        Logger.error({ msg: 'sha256FoldingRule: Invalid previousOutput', previousOutput, numPositions });
    ...
    if (typeof numPositions !== 'number' || numPositions <= 0 || numPositions > 4096) {
        Logger.error({ msg: 'sha256FoldingRule: Invalid numPositions', numPositions });
    ...
    try {
        ...
        try { Logger.error({ msg: 'sha256FoldingRule: sha256Hash failed', error: e, previousOutput, numPositions }); } catch {};
    ...
    try { Logger.logFolding({ previousOutput, rule: 'sha256', positions }); } catch {} // fire-and-forget, robust
    return positions;
}
...
export async function getInitialPositions(numPositions, seed = 'beacon') {
    if (typeof numPositions !== 'number' || numPositions <= 0 || numPositions > 4096) {
        Logger.error({ msg: 'getInitialPositions: Invalid numPositions', numPositions });
    ...
    if (typeof seed !== 'string' || seed.length < 1 || seed.length > 128) {
        Logger.error({ msg: 'getInitialPositions: Invalid seed', seed });
    ...
    try {
        ...
        try { Logger.error({ msg: 'getInitialPositions: sha256Hash failed', error: e, seed }); } catch {};
    ...
    try { Logger.logFolding({ seed, rule: 'initial', positions }); } catch {} // fire-and-forget
    return positions;
}
...
export async function updatePositions(previousOutput, rule, numPositions) {
    ...
    if (rule !== 'sha256') {
        try { Logger.error({ msg: 'updatePositions: Unsupported folding rule', rule }); } catch {};
...
export async function recursiveFolding(blocks, initialOutput, rule, iterations, numPositions) {
    ...
    if (!Array.isArray(blocks) || blocks.length === 0 || blocks.length > 32 || !blocks.every(b => b && typeof b.hash === 'string' && /^[0-9a-fA-F]{64}$/.test(b.hash))) {
        try { Logger.error({ msg: 'recursiveFolding: Invalid blocks', blocks }); } catch {};
    ...
    if (typeof initialOutput !== 'string' || !/^[01]+$/.test(initialOutput) || initialOutput.length < 8) {
        try { Logger.error({ msg: 'recursiveFolding: Invalid initialOutput', initialOutput }); } catch {};
    ...
    if (typeof iterations !== 'number' || iterations < 1 || iterations > 32) {
        try { Logger.error({ msg: 'recursiveFolding: Invalid iterations', iterations }); } catch {};
        throw new FoldingValidationError('recursiveFolding: iterations must be 1-32', { iterations });
    }
    if (typeof numPositions !== 'number' || numPositions <= 0 || numPositions > 4096) {
        try { Logger.error({ msg: 'recursiveFolding: Invalid numPositions', numPositions }); } catch {};
    ...
    for (let i = 1; i <= iterations; i++) {
        try {
            ...
            const { bitstring, auditTrail } = await extractBits(
                blocks,
                safePositions,
                (anomaly, details) => { try { Logger.logExtraction({ anomaly, ...details }); } catch {} },
                { iteration: i }
            );
            try { Logger.logExtraction({ iteration: i, positions: safePositions.slice(), bitstring }); } catch {} // fire-and-forget, robust
            if (!bitstring || bitstring.length === 0) {
                try { Logger.error({ msg: 'recursiveFolding: No bits extracted', iteration: i, positions: safePositions, blocks }); } catch {};
            ...
            try { Logger.error({ msg: 'recursiveFolding: Error in iteration', error: e, iteration: i, rule, numPositions }); } catch {};
