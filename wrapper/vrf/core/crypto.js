...
// Async SHA-256 (WebCrypto in browser, Node.js crypto in Node)
...
export async function sha256Hash(input) {
    ...
    if (typeof window !== 'undefined' && window.crypto && window.crypto.subtle && window.crypto.subtle.digest) {
        try {
            ...
            Logger.error({ msg: 'sha256Hash (browser) failed', error: e });
            throw e;
        }
    }
    throw new Error('sha256Hash: No supported crypto backend found (browser only)');
}

// Convert hex string to byte array (validates input)
export function hexToBytes(hex) {
    if (typeof hex !== 'string' || hex.length % 2 !== 0 || !/^[0-9a-fA-F]+$/.test(hex)) {
        Logger.error({ msg: 'hexToBytes: Invalid hex input', hex });
    ...
    for (let i = 0; i < hex.length; i += 2) {
        ...
        if (isNaN(byte)) {
            Logger.error({ msg: 'hexToBytes: Invalid byte', index: i, byte: hex.substr(i, 2) });
...
export function getBitFromHash(hexHash, pos) {
    if (typeof hexHash !== 'string' || hexHash.length !== 64 || !/^[0-9a-fA-F]+$/.test(hexHash)) {
        Logger.error({ msg: 'getBitFromHash: Invalid hexHash', hexHash });
        throw new Error('getBitFromHash: hexHash must be 64-char hex string');
    }
    if (typeof pos !== 'number' || pos < 0 || pos > 255) {
        Logger.error({ msg: 'getBitFromHash: Invalid pos', pos });
...
export async function runSha256TestVectors() {
    const vectors = [
        ...
        { input: 'The quick brown fox jumps over the lazy dog.', expected: 'ef537f25c895bfa782526529a9b63d97aa631564d5d789c2b765448c8635fb6c' },
    ];
    for (const v of vectors) {
        const actual = await sha256Hash(v.input);
        if (actual !== v.expected) {
            Logger.error({ msg: 'SHA-256 test vector failed', input: v.input, expected: v.expected, got: actual });
            throw new Error('SHA-256 test vector failed');
        }
    }
    Logger.info({ msg: 'SHA-256 test vectors passed' });
}
