...
export async function getKaspaBlocks(n = KASPA_BLOCK_COUNT) {
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        const blockdagResp = await fetch(
          kaspaConfig.endpoint + "/info/blockdag",
        );
        if (!blockdagResp.ok)
          throw new Error(
            `HTTP ${blockdagResp.status}: ${blockdagResp.statusText}`,
          );
        const blockdagData = await blockdagResp.json();
        const tipHashes = blockdagData.tipHashes;
        if (!Array.isArray(tipHashes) || tipHashes.length === 0)
          throw new Error("No tip hashes found");
        const tipHash = tipHashes[0];
        const tipResp = await fetch(
          `${kaspaConfig.endpoint}/blocks/${tipHash}?verbose=true`,
        );
        ...
        while (blocks.length < n) {
          ...
          const confirmations =
            tipBlueScore && blueScore ? tipBlueScore - blueScore + 1 : 0;
          ...
          if (
            selectedParentObj &&
            selectedParentObj.parentHashes &&
            selectedParentObj.parentHashes.length > 0
          ) {
            selectedParentHash = selectedParentObj.parentHashes[0];
          } else if (
            parents.length > 0 &&
            parents[0].parentHashes &&
            parents[0].parentHashes.length > 0
          ) {
            selectedParentHash = parents[0].parentHashes[0];
          }
          if (!selectedParentHash) break;
          const parentResp = await fetch(
            `${kaspaConfig.endpoint}/blocks/${selectedParentHash}?verbose=true`,
          );
        ...
        lastErr = err;
        logError(`Kaspa fetch attempt ${attempt} failed`, {
          n,
          error: err.message,
        });
