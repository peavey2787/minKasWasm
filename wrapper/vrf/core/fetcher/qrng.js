...
export async function getQRNG(providerName = 'nist', length = 16) {
  ...
  if (cache && cache.result && cache.provider === providerName && cache.length === length) {
    if (now - cache.timestamp < CONFIG.QRNG_API_THROTTLE) {
      logInfo('QRNG API throttled, returning cached data', { providerName, length });
      return cache.result;
    }
  }
  try {
    validateProvider(providerName);
    if (!Number.isInteger(length) || length <= 0) throw new Error('QRNG length must be a positive integer');
    // If cache is still valid, return it
    if (cache && cache.result && (now - cache.timestamp < CONFIG.QRNG_CACHE_DURATION) && cache.provider === providerName && cache.length === length) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (Array.isArray(result.data) && result.data.length > 0) {
          ...
          logError('QRNG provider returned invalid data, cache cleared', { providerName, length });
          throw new Error('QRNG provider returned invalid data');
        }
      } catch (err) {
        lastErr = err;
        logError(`QRNG fetch attempt ${attempt} failed`, { providerName, length, error: err.message });
...
function sanitizeQrngResult(result) {
  ...
  if (!result || typeof result !== 'object') return { data: [], length: 0, provider: '' };
  // Only allow expected fields (adjust as needed for your QRNG schema)
  return {
    data: Array.isArray(result.data) ? result.data : [],
    length: Number.isInteger(result.length) ? result.length : (Array.isArray(result.data) ? result.data.length : 0),
