...
export function blockFrequencyTest(bits, blockSize) {
    ...
    if (n === 0 || blockSize <= 0 || blockSize > n) {
        return {
            ...
            pValue: null,
            threshold: 0.01,
            details: { error: 'Invalid input: empty bitstring or blockSize out of range.' }
        };
    }
...
export function longestRunOfOnesTest(bits, blockSize) {
    ...
    if (blockSize === 8) {
        ...
        v = [4, 5, 6, 7];
        pi = [0.1174, 0.2430, 0.2493, 0.3903];
...
export function serialTest(bits, m) {
    ...
    if (n === 0 || m < 1 || m > 16) {
        return {
            ...
            pValue: null,
            threshold: 0.01,
            details: { error: 'Invalid input: empty bitstring or pattern length out of range.' }
        };
    }
...
export function approximateEntropyTest(bits, m) {
    ...
    if (n === 0 || m < 1 || m > 16) {
        return {
            ...
            pValue: null,
            threshold: 0.01,
            details: { error: 'Invalid input: empty bitstring or block size out of range.' }
        };
    }
...
export function cumulativeSumsTest(bits, mode) {
    ...
    const ITMAX = (typeof cumulativeSumsTest.ITMAX === 'number') ? cumulativeSumsTest.ITMAX : 10000;
