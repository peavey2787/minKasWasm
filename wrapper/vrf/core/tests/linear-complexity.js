...
export function linearComplexityTest(bits, blockSize) {
    ...
    function berlekampMasseyGF2(seq) {
        const C = new Array(M).fill(0); C[0] = 1;     // connection polynomial
        const B = new Array(M).fill(0); B[0] = 1;     // previous C
    ...
    for (let b = 0; b < N; b++) {
        ...
        // (0.5, 1.5], (1.5, 2.5], (2.5, ∞)
        if (T <= -2.5) counts[0]++;               // v0
        else if (T <= -1.5) counts[1]++;          // v1
        else if (T <= -0.5) counts[2]++;          // v2
        else if (T <= 0.5) counts[3]++;           // v3
        else if (T <= 1.5) counts[4]++;           // v4
        else if (T <= 2.5) counts[5]++;           // v5
        else counts[6]++;                         // v6
    }
