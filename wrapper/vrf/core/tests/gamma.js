...
export function logGamma(z) {
    if (typeof z !== 'number' || !isFinite(z)) {
        throw new GammaValidationError('logGamma: z must be a finite number', { z });
    }
    const originalZ = z;
...
export function gammaQ(a, x, opts = {}) {
    ...
    if (typeof a !== 'number' || !isFinite(a) || a <= 0) {
        throw new GammaValidationError('gammaQ: a must be a positive finite number', { a });
    }
    if (typeof x !== 'number' || !isFinite(x) || x < 0) {
        throw new GammaValidationError('gammaQ: x must be a non-negative finite number', { x });
    }
    if (opts && (('eps' in opts && (!isFinite(opts.eps) || opts.eps <= 0)) || ('itmax' in opts && (!isFinite(opts.itmax) || opts.itmax <= 0)))) {
        throw new GammaValidationError('gammaQ: opts.eps and opts.itmax must be finite and > 0', { opts });
    }
