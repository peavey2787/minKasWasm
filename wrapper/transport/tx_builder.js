...
export async function buildPendingTransaction({
  ...
  console.log("[buildPendingTransaction] Calling SDK createTransactions with:", {
  ...
  if (entries.length > 0) {
    ...
    console.log("[buildPendingTransaction] Entry analysis:", {
      isWasmObject,
      hasEntryGetter,
    ...
    if (isWasmObject) {
      ...
      // Plain JS objects from RPC - wrap in UtxoEntries to convert to WASM
      console.log("[buildPendingTransaction] Converting plain JS entries to WASM UtxoEntries");
  ...
  console.log("[buildPendingTransaction] Outputs:", outputs.map(o => ({
  ...
  try {
    ...
    console.error("[buildPendingTransaction] SDK createTransactions error:", err.message || err);
    throw err;
  }
...
export async function buildManualTransaction({
  ...
  console.log(`[buildManualTransaction] entries=${entries.length}, totalInput=${totalInput}`);
  ...
  console.log(`[buildManualTransaction] totalOutput=${totalOutput}, estimatedFee=${estimatedFee}, change=${change}`);
