...
(async function runQrngUnitTests() {
  ...
  assert.ok(result && Array.isArray(result.data), 'getQRNG returns object with data array');
  assert.equal(result.data.length, 4, 'getQRNG returns correct data length');

  // 2. Throws on invalid length
  let threw = false;
  try { await getQRNG('anu', 0); } catch (e) { threw = true; }
  ...
  assert.ok(Array.isArray(result2.data), 'throttled getQRNG returns data array');
