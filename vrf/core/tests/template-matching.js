...
export function nonOverlappingTemplateTest(bits, template) {
    ...
    for (let b = 0; b < N; b++) {
        ...
        while (j + m <= end) {
            ...
            let match = true;
            for (let k = 0; k < m; k++) {
                if (bits[j + k] !== template[k]) { match = false; break; }
            }
            if (match) {
                counts[b]++;
...
export function overlappingTemplateTest(bits, template) {
    ...
    for (let i = 0; i < m; i++) {
        if (template[i] !== '1') {
            return {
                ...
                pValue: null,
                threshold: 0.01,
                details: { m, error: "Unsupported template: NIST defines overlapping test for template '111...1' only." }
            };
        }
    }
    ...
    for (let i = 0; i + m <= n; i++) {
        let match = true;
        for (let k = 0; k < m; k++) {
            if (bits[i + k] !== '1') { match = false; break; }
        }
        if (match) W++;
    }
